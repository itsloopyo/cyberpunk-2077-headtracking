...
:: Pinned off before the parser in :main runs. With delayed expansion on,
:: cmd.exe strips a `!` out of the expanded text of `set "_ARG=%~1"`, so a real
:: game path like C:\Games\Oh! My Game loses the `!`, `if exist` fails, and a
:: valid folder is rejected with exit 2.
...
:: which is `uninstall.cmd "<path>" /y`, lopari's exact call shape.
::
:: Quoted-string form is required here - bracket form `if [%~1]==[/y]` does NOT
:: quote, so a path arg containing whitespace ("C:\...\Gone Home") splits across
:: the brackets and crashes cmd with "[Home]==[/y] was unexpected at this time".
:: The trailing-backslash hazard the bracket form was working around is moot
